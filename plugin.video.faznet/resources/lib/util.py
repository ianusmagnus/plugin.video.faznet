...
def addVideoMenuItem(caption, duration, link, icon=None, thumbnail=None, folder=False):
    """
...
def addMenuItem(caption, link):
    ...
    Returns True if the item is successfully added, False otherwise
    """
    listItem = xbmcgui.ListItem(unicode(caption))
    listItem.setInfo(type="Video", infoLabels={ "Title": caption })
    return xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=link, listitem=listItem, isFolder=True)
